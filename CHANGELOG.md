...
- Add GitHub Actions workflows based on go-simpler's shared lint, test, and vulnerability scans; the test workflow runs against stable Go on Linux and macOS, golangci-lint is upgraded to the latest stable v2 release, and the module now targets Go 1.25.8 so govulncheck uses the fixed standard library ([#260]).
